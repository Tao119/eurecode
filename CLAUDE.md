...
## コミット・プッシュ前の必須チェック（重要）

**push前に必ず以下を確認すること：**

### 0. 選択的コミット（最重要）
- **自分の作業内容の必要部分のみをコミットすること**
- **`git add -A` や `git add .` でunstaged全てをコミットしては絶対にダメ**
- 各ファイルの変更内容を確認し、意図した変更のみをステージング
- `git add <ファイル名>` で個別にステージング

### 1. ビルド確認
```bash
pnpm build
```
ビルドが成功するまでpush禁止。TypeScriptエラーやモジュール不足などを事前に検出する。

### 2. コードレビュー（diff確認）
codex CLI または code-reviewer エージェントでdiffベースのレビューを実施。

### 3. UI変更がある場合
Playwright MCPを使用して動作確認を必ず行う：
- 変更したページを実際にブラウザで確認
- 基本的な操作が正常に動作することを検証
- レイアウト崩れがないことを確認

### チェックリスト
- [ ] 必要な変更のみを選択的にステージング（git add -A 禁止）
- [ ] `pnpm build` が成功する
- [ ] diffをレビューして意図通りか確認
- [ ] UI変更がある場合、Playwrightで動作確認済み
- [ ] コミットメッセージが適切
